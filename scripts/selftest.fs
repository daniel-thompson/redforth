...
( --------------------------------------------------------------------------- )
." File read/write ..."
S" doesnotexist" R/O OPEN-FILE 0<> ASSERT DROP

VARIABLE OUTPUT_FILE
S" filetest.txt" W/O CREATE-FILE 0= ASSERT OUTPUT_FILE !
...
VARIABLE OUTPUT_FILE
S" filetest.txt" W/O OPEN-FILE 0= ASSERT OUTPUT_FILE !
S" CHUNK#4 " OUTPUT_FILE @ WRITE-FILE 0= ASSERT
OUTPUT_FILE @ CLOSE-FILE 0= ASSERT
FORGET OUTPUT_FILE

VARIABLE INPUT_FILE
S" filetest.txt" R/O OPEN-FILE 0= ASSERT INPUT_FILE !
HERE @ 8 INPUT_FILE @ READ-FILE 0= ASSERT 8 = ASSERT
HERE @ 6 + C@ CHAR 1 = ASSERT
HERE @ 8 INPUT_FILE @ READ-FILE 0= ASSERT 8 = ASSERT
HERE @ 6 + C@ CHAR 2 = ASSERT
HERE @ 8 INPUT_FILE @ READ-FILE 0= ASSERT 8 = ASSERT
HERE @ 6 + C@ CHAR 3 = ASSERT
( Give an oversize buffer for the last read to check we get the right return )
HERE @ 1024 INPUT_FILE @ READ-FILE 0= ASSERT 8 = ASSERT
...
ERROR_COUNT @ 0= [IF]
	CR ." All tested passed OK" CR
	BYE
[ELSE]
	CR ." SELFTEST COMPLETED WITH " ERROR_COUNT @ . ." FAILURES"
	1 FN-exit CCALL1
[THEN]
